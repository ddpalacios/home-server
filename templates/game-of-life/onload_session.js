	...
	*/
    ...
    async function get_session_by_sessionId(sessionId){
		var request = new Request('game-of-life/session?Id='+sessionId, {
							method: 'GET',
							headers: new Headers({
    ...
    async function get_session_by_userId(userId){
		var request = new Request('game-of-life/session?userId='+userId, {
							method: 'GET',
							headers: new Headers({
										'Accept': 'application/json'
									})
    ...
    async function start_page(user){
        if (window.location.hash){
            ...
            var request = new Request('game-of-life/session?userId='+user['Id'], {
                                        method: 'GET',
                                        headers: new Headers({
                                                    'Accept': 'application/json'
                                                })});
    ...
    async function create_user(){
            var request = new Request('/game-of-life/user', {
                                method: 'POST',
                                headers: new Headers({
                                            'Accept': 'application/json'
                                        })
    ...
    async function create_session(sessionName, user){
         ...
         var request = new Request('/game-of-life/session', {
                            method: 'POST',
                            headers: new Headers({
                                        'Accept': 'application/json',
    ...
    function display_invitation_link(chatbox_contents_div){
        ...
        cpy_btn.style.fontSize = '20px'
        cpy_btn.style.width = '100%'
        cpy_btn.style.color = 'white'
        cpy_btn.textContent = "Copy Invitation URL"
        cpy_btn.style.backgroundColor = 'rgba(4, 121, 4, 1)';
                cpy_btn.addEventListener('mouseover', function () {
                cpy_btn.style.backgroundColor = 'rgba(2, 174, 2, 1)';
                });
    ...
    function display_session_button(){
        ...
        stop_session_btn.style.width = '100%'
        stop_session_btn.style.fontSize = '20px'
        let default_color =  'rgba(255, 251, 1, 1)'
        let onhover_color = 'rgba(255, 251, 1, 0.6)'
        if (isHost == 'false'){
            stop_session_btn.textContent = "Leave Session"
            default_color =   'rgba(255, 251, 1, 1)'
            onhover_color = 'rgba(255, 251, 1, 0.6)'
    ...
    function load_messages(current_user, sessionId){
        let data_request = new Request('/game-of-life/session/messages?sessionId='+sessionId, {
                    method: 'GET'
                    ,headers: new Headers({
                        'Accept': 'application/json'
                        })
    ...
    function display_session(sessionName,current_user){
        ...
        send_btn.style.fontSize = '20px'
        send_btn.style.color = 'white'
        send_btn.style.width = '100%'
        ...
        send_btn.style.backgroundColor ='rgba(3, 152, 53, 1)';
    ...
    document.getElementById("chatbox_btn").onclick = function(){
        ...
        if (chatbox.style.display != 'none'){
            ...
            chatbox_btn.style.backgroundColor = 'rgb(189, 1, 1)'
            chatbox_btn.addEventListener('mouseover', function () {
