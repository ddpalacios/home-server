class Clean_Activity extends Activity{
    constructor(flowchart,activity){
        super(flowchart, activity)
        this.add_button_label = "+ Add Clean"
        this.clean_operations = ['TRIM', 'LOWER', 'UPPER']
        this.settings = this.get_settings_element()
    }
    get_operation_settings(){
        let settings = super.get_operation_settings('clean')
    ...
    _add_column(e, widget, activity){
        ...
        let settings = [
            ...
            {
                'type': 'selector'
                ,'options': this.clean_operations
                ,'default_value': this.clean_operations[0]
                ,'name': 'operation'
            },
            {
                'type': 'span'
