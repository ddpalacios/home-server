class Window_Activity extends Activity{
    constructor(flowchart,activity){
        super(flowchart, activity)
        this.add_button_label = "+ Add Rank"
        this.rank_operations = ['ROW_NUMBER', 'RANK', 'DENSE_RANK']
        this.settings = this.get_settings_element()
    }
    get_operation_settings(){
        let settings = super.get_operation_settings('window')
    ...
    _add_column(e, widget, activity){
        ...
        let settings = [
            ...
            {
                'type': 'selector'
                ,'options':this.rank_operations
                ,'default_value': this.rank_operations[0]
                ,'name': 'operation'
            },
            {
                'type': 'span'
