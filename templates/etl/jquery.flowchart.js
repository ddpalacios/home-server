...
// function key_name_change(activities,activity,obj,element){
...
//         activities[activity['operatorId']].properties['output_value'] = obj

//     }
// }

// async function readText(activities, activity, event) {
...
//     // activities[activity['operatorId']].properties['output_value'] = obj
    ...
    //             key_name_change(activities,activity,obj,this)
    ...
    //     let data_types = get_dataTypeSelector_element(typeof obj[key])
    ...
    $.widget("flowchart.flowchart", {
        ...
        getData: function (Id) {
            ...
            Object.keys(this.data.operators).forEach(element => {
                ...
                operator_copy['left'] = this.data.operators[element].left
                operator_copy['operatorId'] = this.data.operators[element].operatorId
                operator_copy['properties'] = {}
                operator_copy['properties']['title']=operator.title 
                operator_copy['properties']['filteType']=operator.fileType 
                operator_copy['properties']['settings']=operator.settings 
                ...
                if (operator.activityType == 'join' || operator.activityType == 'append'){
                        operator_copy['properties']['inputs'] =  {
                                                    ...
                                                    input_2: { value: null
                                                    ,label: operator.inputs.input_2.label
                                                    }
                                                };
                        operator_copy['properties']['outputs']= {
                            output: { value: null 
                                ,label: operator.outputs.output.label
                    ...
                    operator_copy['properties']['inputs']= {
                        input: { value: null
                            ,label: operator.inputs.input.label
                        }
                        };
                ...
                pipeline['activities'][operator_copy.operatorId] = {
                    'top': operator_copy.top,
                    'operatorId': operator_copy.operatorId,
                    'left':operator_copy.left
                    ,'properties':operator_copy.properties
                }
            });
        ...
        getOperatorActivity: function (operatorId) {
            this.data.operators[operatorId].internal.properties['operatorId'] = operatorId
            var to = this.getLinksFrom(operatorId)
            var from = this.getLinksTo(operatorId)

            all_from_links = []
            for (let i=0; i<from.length; i++){
                let target_from_operator = this.data.operators[from[i].fromOperator].internal.properties  
                target_from_operator['operatorId'] = from[i].fromOperator
                all_from_links.push(target_from_operator)
            }
            all_to_links = []
        ...
        getOperators: function(){
            let operations = this.data.operators
            return operations;
        },
