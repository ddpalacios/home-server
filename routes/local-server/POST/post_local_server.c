...
/* PUT proxy. Identical to delete_to_local apart from the request verb:
 * forwards the Cookie + X-Forwarded-Host headers and the JSON body to the
 * upstream so authenticated PUT routes (e.g. /me/agents/<id>/memory/facts/
 * <fact_id>) resolve the session and receive their payload intact. */
void put_to_local(struct Socket* socket,char* http_header, char*body, char* route, const char* port){
    ...
    for (;;) {
        ...
        if (bytes_recved <= 0)
            break;
        if (total + (size_t)bytes_recved > MAX_LOCAL_RESPONSE) {
            printf("put_to_local: upstream response too large, aborting\n");
            free(response);
            close(sfd);
            send_response_code(socket->cSSL, 502);
            return;
